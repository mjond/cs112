/* VecTester.cpp defines the unit test-methods for Vec, a simple vector clas
...
void VecTester::subsciptRetriveTester() const{
	// nonempty case, valid subscript
	cout << "Testing [ ] retrieve..." << flush;
	Vec v(5);
	for (unsigned i = 0; i < 5; i++) {
		v.setItem(i, i);
	}
...
void VecTester::subscriptSetTester() const {
	cout << "Testing [ ] change..." << flush;
	Vec v(5);
	for (unsigned i = 0; i < 5; i++) {
		v[i] = i + 1;
	}
